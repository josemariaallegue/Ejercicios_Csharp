/*
Script de implementación para sql_data_tools_personas-direccion

Una herramienta generó este código.
Los cambios realizados en este archivo podrían generar un comportamiento incorrecto y se perderán si
se vuelve a generar el código.
*/

GO
SET ANSI_NULLS, ANSI_PADDING, ANSI_WARNINGS, ARITHABORT, CONCAT_NULL_YIELDS_NULL, QUOTED_IDENTIFIER ON;

SET NUMERIC_ROUNDABORT OFF;


GO
:setvar DatabaseName "sql_data_tools_personas-direccion"
:setvar DefaultFilePrefix "sql_data_tools_personas-direccion"
:setvar DefaultDataPath "C:\Users\alleg\AppData\Local\Microsoft\Microsoft SQL Server Local DB\Instances\MSSQLLocalDB\"
:setvar DefaultLogPath "C:\Users\alleg\AppData\Local\Microsoft\Microsoft SQL Server Local DB\Instances\MSSQLLocalDB\"

GO
:on error exit
GO
/*
Detectar el modo SQLCMD y deshabilitar la ejecución del script si no se admite el modo SQLCMD.
Para volver a habilitar el script después de habilitar el modo SQLCMD, ejecute lo siguiente:
SET NOEXEC OFF; 
*/
:setvar __IsSqlCmdEnabled "True"
GO
IF N'$(__IsSqlCmdEnabled)' NOT LIKE N'True'
    BEGIN
        PRINT N'El modo SQLCMD debe estar habilitado para ejecutar correctamente este script.';
        SET NOEXEC ON;
    END


GO
USE [$(DatabaseName)];


GO
PRINT N'La siguiente operación se generó a partir de un archivo de registro de refactorización a9140265-73e6-42f9-92fe-ae026dc8820a';

PRINT N'Cambiar el nombre de [dbo].[Direccion].[PersonId] a PersonaId';


GO
EXECUTE sp_rename @objname = N'[dbo].[Direccion].[PersonId]', @newname = N'PersonaId', @objtype = N'COLUMN';


GO
PRINT N'Creando [dbo].[PersonaCompleta]...';


GO
CREATE VIEW [dbo].[PersonaCompleta]
	as
	select [p].[Nombre], [p].[Apellido]
		, [a].[PersonaId], [a].[DireccionCalle], [a].[Ciudad], [a].[Provincia], [a].[CodigoPostal]
	from dbo.Persona as p
	left join dbo.Direccion as a on p.Id = a.PersonaId
GO
PRINT N'Creando [dbo].[spPersona_FiltroPorApellido]...';


GO
CREATE PROCEDURE [dbo].[spPersona_FiltroPorApellido]
	@apellido nvarchar(20)
AS
	
begin
	SELECT p.[Id], p.[Nombre], p.[Apellido]
	from dbo.Persona as p
	where p.Apellido = @apellido
end
GO
-- Paso de refactorización para actualizar el servidor de destino con los registros de transacciones implementadas
IF NOT EXISTS (SELECT OperationKey FROM [dbo].[__RefactorLog] WHERE OperationKey = 'a9140265-73e6-42f9-92fe-ae026dc8820a')
INSERT INTO [dbo].[__RefactorLog] (OperationKey) values ('a9140265-73e6-42f9-92fe-ae026dc8820a')

GO

GO
PRINT N'Actualización completada.';


GO
